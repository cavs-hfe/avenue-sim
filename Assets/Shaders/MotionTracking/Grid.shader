    ...
    SubShader {
      ...
      Pass {
        ...
        float4 frag(vertexOutput input) : COLOR {
          if (frac((input.worldPos.x + _GridOffsetX)/_GridSpacingX) < (_GridThickness / _GridSpacingX) ||  //x/y plane
		   frac((input.worldPos.y + _GridOffsetY)/_GridSpacingY) < (_GridThickness / _GridSpacingY)) {
		    return _GridColour;
		  } 
		  else if (frac((input.worldPos.x + _GridOffsetX)/_GridSpacingX) < (_GridThickness / _GridSpacingX) ||  //x/z plane
		   frac((input.worldPos.z + _GridOffsetY)/_GridSpacingY) < (_GridThickness / _GridSpacingY)) {	
		    return _GridColour;
		  }
		  else if (frac((input.worldPos.y + _GridOffsetX)/_GridSpacingX) < (_GridThickness / _GridSpacingX) ||  //y/z plane
		   frac((input.worldPos.z + _GridOffsetY)/_GridSpacingY) < (_GridThickness / _GridSpacingY)) {
            return _GridColour;
          }
          else {
